...
exports.checkBadgesOnTaskApproval = exports.aggregateLeaderboard = exports.streakWarning = exports.dailyStreakUpdate = exports.aiProductPicker = exports.aiFraudDetector = exports.aiContentReviewer = exports.aiEarningsPredictor = exports.aiTaskGenerator = void 0;
 ...
 */
async function callDeepSeek(uid, prompt, cacheKey, ttlSeconds, systemInstruction) {
    var _a, _b;
    const now = Date.now();
 ...
 */
exports.aiTaskGenerator = functions.pubsub.schedule('0 6 * * *')
    ...
    for (const userDoc of activeUsersSnap.docs) {
        ...
        try {
            ...
            result.tasks.forEach((task) => {
                ...
                batch.set(taskRef, Object.assign(Object.assign({}, task), { earning: task.earnAmount, status: 'assigned', createdAt: admin.firestore.FieldValue.serverTimestamp(), isAI: true }));
            });
 ...
 */
exports.aiEarningsPredictor = functions.https.onCall(async (data, context) => {
    if (!context.auth)
 ...
 */
exports.aiContentReviewer = functions.firestore
 ...
 */
exports.aiFraudDetector = functions.pubsub.schedule('0 */6 * * *')
    .onRun(async (context) => {
 ...
 */
exports.aiProductPicker = functions.https.onCall(async (data, context) => {
    if (!context.auth)
        throw new functions.https.HttpsError('unauthenticated', 'Login required');
    const uid = context.auth.uid;
    const userDoc = await db.collection('users').doc(uid).get();
 ...
 */
exports.dailyStreakUpdate = functions.pubsub.schedule('0 0 * * *')
    .timeZone('Asia/Kolkata')
    .onRun(async (context) => {
 ...
 */
exports.streakWarning = functions.pubsub.schedule('0 21 * * *')
    .timeZone('Asia/Kolkata')
    .onRun(async (context) => {
 ...
 */
exports.aggregateLeaderboard = functions.pubsub.schedule('0 * * * *')
    .onRun(async (context) => {
 ...
 */
exports.checkBadgesOnTaskApproval = functions.firestore
    .document('taskSubmissions/{submissionId}')
